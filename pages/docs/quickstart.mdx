...
When running any executable file on another computer, make sure that the computer is running the same operating system
as the computer that compiled the executable. For example, if you compile your program on Windows, you cannot run it on
a computer running macOS.

We will discuss **cross-compilation** in a later section, which allows you to compile your program for a different
operating system than the one you are currently using, solving this problem.
...
No. You can write top-level Terbium code without a `main` function. However, it is not advised to do so unless you are
writing a library or other code that is not meant to be run as a standalone program. In fact, the `main` function is:
